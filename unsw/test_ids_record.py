...
class FlowState:
    ...
    def to_feature_row(self, min_pkts=3, min_nonzero=5):
        ...
        # ---- Validity check ----
        num_nonzero = sum(1 for k,v in row.items() if isinstance(v,(int,float)) and v != 0)
        #row["valid_flow"] = self.pkt_cnt >= min_pkts and num_nonzero >= min_nonzero
        row["valid_flow"] = True  # for now, accept all flows
...
def collector_thread(flow_table: FlowTable, batch_q: queue.Queue, stop_event: threading.Event):
    ...
    while not stop_event.is_set():
        ...
        for fs in frozen:
            row = fs.to_feature_row()
            if not row.get("valid_flow", False):
                continue  # skip flows with too few packets or features
            batch.append(row)
...
OUTPUT_CSV = "realtime_predictions.csv"
...
def inference_thread(batch_q: queue.Queue, preproc: Preprocessor, predictor: Predictor,
    ...
    if not os.path.exists(OUTPUT_CSV):
        pd.DataFrame(columns=["src","dst","sport","dport","proto","pred_label"]).to_csv(OUTPUT_CSV, index=False)
    ...
    while not stop_event.is_set():
        ...
        # Keep identity columns for printing / saving
        ...
        try:
            ...
            if view is not None:
                out = view.copy()
                # Only include valid flows
                valid_records = [
                    (lbl, row.get("valid_flow", True)) for lbl, row in zip(y_labels, df.to_dict("records"))
                ]
                # Apply low-conf mark and filter
                out["pred_label"] = [
                    f"{lbl} (low-conf)" if not valid else lbl
                    for lbl, valid in valid_records
                ]

                # Save all valid flows to CSV
                valid_out = out[[col for col in out.columns]]  # copy all columns
                valid_out.to_csv(OUTPUT_CSV, mode="a", header=False, index=False)

                # Print top 20
                if print_labels:
                    for _, row in out.head(20).iterrows():
                        label = row["pred_label"]
                        color = LABEL_COLORS.get(label.split(" ")[0], Fore.WHITE)
